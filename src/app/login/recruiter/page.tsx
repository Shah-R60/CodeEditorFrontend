  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/db/users/login`, {
        ...
        body: JSON.stringify({ email: data.email, password: data.password, role: "RECRUITER" })
      });
    ...
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-6">
      ...
      <div className="w-full max-w-md bg-gray-900/80 backdrop-blur-xl rounded-3xl border border-gray-800 p-8 shadow-2xl relative z-10">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white tracking-tight">Recruiter Portal</h1>
          <p className="text-sm text-gray-400 mt-2 text-center">
            Sign in to manage coding assessments and candidate interviews.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div className="flex items-center justify-between text-sm">
            <label className="flex items-center cursor-pointer">
              <input type="checkbox" className="w-4 h-4 rounded border-gray-700 bg-gray-800 text-emerald-500 focus:ring-emerald-500 focus:ring-offset-gray-900" />
              <span className="ml-2 text-gray-400">Remember me</span>
            </label>
            <a href="#" className="text-emerald-400 hover:text-emerald-300 transition">Forgot password?</a>
          </div>

          <button
            type="submit"
            disabled={isLoading}
            className="w-full flex justify-center items-center gap-2 bg-emerald-500 text-gray-950 font-bold py-3.5 px-4 rounded-xl hover:bg-emerald-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed group"
          >
            {isLoading ? (
              <>
        ...
        <div className="mt-8 text-center text-sm text-gray-400">
          ...
          <Link href="/signup/recruiter" className="text-emerald-400 hover:text-emerald-300 font-medium transition">
            Sign up here
          </Link>
        </div>
      </div>
