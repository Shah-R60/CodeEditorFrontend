  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/db/users/google-login`, {
        ...
        body: JSON.stringify({ idToken, role: "RECRUITER" })
      });
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans text-slate-900 selection:bg-blue-100">
      <div className="sm:mx-auto sm:w-full sm:max-w-md flex flex-col items-center">
        ...
        <p className="mt-2 text-center text-sm text-slate-600">
          Sign in to manage coding assessments and interviews
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-12 px-4 shadow-xl shadow-slate-200/50 sm:rounded-2xl sm:px-10 border border-slate-100 text-center">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full flex justify-center items-center gap-3 py-3 px-4 border border-slate-200 rounded-xl shadow-sm text-sm font-bold text-slate-700 bg-white hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed group"
          >
            {isLoading ? (
              <>
                <Loader2 className="animate-spin text-blue-600" size={20} /> Authenticating...
              </>
            ) : (
              <>
