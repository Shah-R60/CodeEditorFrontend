...
import { Lock, User as UserIcon, Mail, Loader2, Code2, ArrowRight } from "lucide-react";
...
export default function StudentSignup() {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      // Redirect to the main coding environment (test page)
      router.push("/");
    } catch (err: any) {
      ...
      <div className="w-full max-w-md bg-gray-900/80 backdrop-blur-xl rounded-3xl border border-gray-800 p-8 shadow-2xl relative z-10">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-gray-400 mt-2 text-center">
            Sign up to take coding assessments and join interviews.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full flex justify-center items-center gap-2 bg-blue-500 text-white font-bold py-3.5 px-4 rounded-xl hover:bg-blue-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed group shadow-[0_0_15px_rgba(59,130,246,0.2)] hover:shadow-[0_0_25px_rgba(59,130,246,0.4)] mt-2"
          >
            {isLoading ? (
              <>
        ...
        <div className="mt-8 text-center text-sm text-gray-400">
          ...
          <Link href="/login/student" className="text-blue-400 hover:text-blue-300 font-medium transition">
            Sign in here
          </Link>
        </div>
      </div>
