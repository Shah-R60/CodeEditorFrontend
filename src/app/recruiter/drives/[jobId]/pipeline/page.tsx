...
export default function PipelinePlanTab() {
  ...
  const currentRoundNum = rounds.length > 0 ? Math.min(maxRoundReached + 1, rounds.length) : 1;
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-lg font-bold text-slate-900">Hiring Rounds</h3>
          <p className="text-sm text-slate-500">Define the assessment flow for this role.</p>
        </div>
        <button 
          onClick={() => setIsModalOpen(true)}
          className="flex items-center gap-2 bg-slate-50 border border-slate-200 hover:border-blue-500 hover:text-blue-700 hover:bg-blue-50 text-slate-700 px-4 py-2 rounded-xl transition-all shadow-sm font-semibold text-sm"
        >
      ...
      <div className="flex flex-col">
        {loading ? (
          ...
          <div key={round.id} className="flex flex-col">
            <div className="bg-white border border-slate-200 rounded-xl p-4 flex items-start gap-4 shadow-sm group hover:border-blue-300 transition-colors">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-slate-50 text-slate-600 text-xs font-medium border border-slate-200">
                    <div className={`w-1.5 h-1.5 rounded-full ${statusColor}`}></div>
                    {status}
                  </div>
                  
                  <span className="px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 text-xs font-medium border border-slate-200 ml-2">
                    {round.type}
                  </span>
                </div>
              ...
              <p className="text-sm text-slate-500 mt-2 ml-9">{round.description}</p>
              ...
              <div className="mt-4 ml-9 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 flex flex-col">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Timeline</span>
                  {round.startDate ? (
                    ...
                    <div className="flex items-center gap-2 text-sm font-medium text-amber-600 bg-amber-50 px-3 py-2 rounded-lg border border-amber-100 w-full mt-auto mb-auto">
                    ...
                    {isInterview ? (
                      <>
                        <div className="flex justify-between items-center text-sm font-medium">
                          ...
                          <span className="text-slate-900 bg-slate-200/50 px-2 py-0.5 rounded-md">{invited}</span>
                        </div>
                        <div className="flex justify-between items-center text-sm font-medium">
                          <span className="text-slate-600">Completed</span>
                          <span className="text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-md">{completed}</span>
                        </div>
                        <div className="flex justify-between items-center text-sm font-medium">
                          <span className="text-slate-600">Pending</span>
                          <span className="text-amber-700 bg-amber-50 px-2 py-0.5 rounded-md">{pending}</span>
                        </div>
                      </>
                    ) : (
                      <>
                        ...
                        <div className="flex justify-between items-center text-sm font-medium">
                          ...
                          <span className="text-blue-700 bg-blue-50 px-2 py-0.5 rounded-md">{completed}</span>
                        </div>
                        <div className="flex justify-between items-center text-sm font-medium">
                          <span className="text-slate-600">Qualified</span>
                          <span className="text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-md">{qualified}</span>
                        </div>
                      </>
                    )}
                  </div>
                  ...
                  {(() => {
                    return (
                      <div className="mt-auto">
                        <div className="w-full bg-slate-200 rounded-full h-3.5 mb-4 overflow-hidden border border-slate-300">
                          <div 
                        ...
                        <div className="flex justify-between items-end">
                          ...
                          <div className="text-sm font-medium text-slate-500 mb-1">
                            <span className="text-slate-800">{finishedMetric}</span> / {totalMetric} Finished
                          </div>
                        </div>
                      </div>
                    );
