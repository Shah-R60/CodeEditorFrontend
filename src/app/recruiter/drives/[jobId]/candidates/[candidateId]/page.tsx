  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-20">
        ...
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <span className={`px-3 py-1 rounded-full text-xs font-bold ${
              candidate.status === 'Passed' ? 'bg-emerald-100 text-emerald-700' :
      ...
      <div className="max-w-6xl mx-auto px-4 mt-8">
        {/* Header Section */}
        <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-slate-200 mb-6 flex flex-col md:flex-row items-start md:items-center gap-6 relative overflow-hidden">
          ...
          <div className="w-24 h-24 bg-gradient-to-br from-amber-500 to-orange-400 rounded-full flex items-center justify-center text-white text-3xl font-bold shadow-md border-4 border-white shrink-0 relative z-10">
            {candidate.name.substring(0, 2).toUpperCase()}
          </div>
          
          <div className="flex-1 relative z-10">
            <h1 className="text-3xl font-bold text-slate-900">{candidate.name}</h1>
            <p className="text-slate-600 mt-1 font-medium">{candidate.email}</p>
          ...
          {candidate.score && (
            <div className="mt-4 md:mt-0 bg-emerald-50 border border-emerald-100 rounded-xl p-4 text-center shrink-0 relative z-10 min-w-[120px]">
              <span className="block text-xs font-bold text-emerald-600 uppercase tracking-wider mb-1">Assessment Score</span>
              <span className="text-2xl font-black text-emerald-700">{candidate.score}</span>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
              <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4">Profile Completeness</h3>
              <div className="w-full bg-slate-100 h-2 rounded-full mb-4 overflow-hidden">
                <div className="bg-emerald-500 h-full rounded-full" style={{ width: resumeData ? '100%' : '20%' }}></div>
              </div>
              <div className="space-y-3">
                <div className="flex items-center gap-3 text-sm">
                  <CheckCircle2 size={18} className={resumeData ? "text-emerald-500" : "text-slate-300"} />
                  <span className={resumeData ? "text-slate-800" : "text-slate-400"}>Resume Uploaded</span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <CheckCircle2 size={18} className={summary ? "text-emerald-500" : "text-slate-300"} />
                  <span className={summary ? "text-slate-800" : "text-slate-400"}>Profile Summary</span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <CheckCircle2 size={18} className={skills.length > 0 ? "text-emerald-500" : "text-slate-300"} />
                  <span className={skills.length > 0 ? "text-slate-800" : "text-slate-400"}>Key Skills</span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <CheckCircle2 size={18} className={experience.length > 0 || internships.length > 0 ? "text-emerald-500" : "text-slate-300"} />
                  <span className={experience.length > 0 || internships.length > 0 ? "text-slate-800" : "text-slate-400"}>Experience / Internships</span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <CheckCircle2 size={18} className={education.length > 0 ? "text-emerald-500" : "text-slate-300"} />
                  <span className={education.length > 0 ? "text-slate-800" : "text-slate-400"}>Education</span>
                </div>
              </div>
            </div>
            ...
            {skills && skills.length > 0 && (
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-bold text-slate-800">Key Skills</h3>
                </div>
                <div className="flex flex-wrap gap-2">
                  {skills.map((s: string, idx: number) => (
                    <span key={idx} className="px-3 py-1.5 bg-slate-50 border border-slate-200 text-slate-700 rounded-lg text-sm font-medium hover:border-amber-300 hover:bg-amber-50 transition-colors">
                      {s}
                    </span>
                  ))}
                </div>
            ...
            {languages && languages.length > 0 && (
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-bold text-slate-800">Languages Known</h3>
                </div>
                <div className="space-y-3">
                  {languages.map((l: string, idx: number) => (
                    <div key={idx} className="flex items-center justify-between border-b border-slate-100 pb-2 last:border-0 last:pb-0">
                      ...
                      <span className="text-xs text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">Proficient</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {candidate.resumeFile && (
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-bold text-slate-800">Original Resume</h3>
                </div>
                <a 
          ...
          <div className="lg:col-span-2 space-y-6">
            {!resumeData ? (
              <div className="bg-white rounded-2xl p-12 shadow-sm border border-slate-200 text-center">
                ...
                <h3 className="text-xl font-bold text-slate-800 mb-2">No AI Profile Available</h3>
                <p className="text-slate-500">This candidate did not upload a resume, or the parsing process failed.</p>
              </div>
            ) : (
              <>
                {summary && (
                  ...
                  <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                    ...
                    <div className="space-y-6">
                      {experience.map((exp: any, idx: number) => (
                        <div key={idx} className="relative pl-6 border-l-2 border-orange-100 last:border-0 last:pb-0 pb-6">
                          <div className="absolute w-3 h-3 bg-orange-400 rounded-full -left-[7.5px] top-1"></div>
                          <h4 className="text-base font-bold text-slate-900">{exp.jobtitle}</h4>
                          <p className="text-sm font-medium text-orange-600 mb-1">{exp.companyname}</p>
                ...
                {internships && internships.length > 0 && (
                  <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                    <div className="flex items-center gap-2 mb-6">
                      ...
                      <h3 className="font-bold text-slate-800">Internships</h3>
                    </div>
                    
                    <div className="space-y-6">
                      {internships.map((intern: any, idx: number) => (
                        <div key={idx} className="relative pl-6 border-l-2 border-pink-100 last:border-0 last:pb-0 pb-6">
                          <div className="absolute w-3 h-3 bg-pink-400 rounded-full -left-[7.5px] top-1"></div>
                          <h4 className="text-base font-bold text-slate-900">{intern.jobtitle}</h4>
                          <p className="text-sm font-medium text-pink-600 mb-1">{intern.companyname}</p>
                          <div className="flex items-center gap-4 text-xs text-slate-400 mb-3 font-medium">
                          ...
                          {intern.description && <p className="text-sm text-slate-600 leading-relaxed">{intern.description}</p>}
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                    ...
                    <div className="space-y-6">
                      {education.map((edu: any, idx: number) => (
                        <div key={idx} className="relative pl-6 border-l-2 border-emerald-100 last:border-0 last:pb-0 pb-6">
                          <div className="absolute w-3 h-3 bg-emerald-400 rounded-full -left-[7.5px] top-1"></div>
                          <h4 className="text-base font-bold text-slate-900">{edu.degree} {edu.specialisation ? `in ${edu.specialisation}` : ''}</h4>
                          <p className="text-sm font-medium text-emerald-600 mb-1">{edu.collegename}</p>
